...
func button_pressed(button_index):
	...
	if button_index == 2:
		if held_object:
			...
			if teleport_mesh != null and not teleport_mesh.visible and not held_object:
	...
	if button_index == 15:
		...
		if teleport_button_down:
			...
			if teleport_pos and teleport_mesh.visible:
				var camera_offset = get_parent().get_node("ARVRCamera").global_transform.origin - get_parent().global_transform.origin
				camera_offset.y = 0
