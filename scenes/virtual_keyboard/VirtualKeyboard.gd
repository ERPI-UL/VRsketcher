...
signal keyboard_input_event(event);
...
func _on_button_down(b):
	...
	emit_signal("keyboard_input_event", ev);
...
func _create_keyboard_buttons():
	...
	_shift_button = _create_button(self, "^", 0, 3, 1, 2);
	_shift_button.toggle_mode = true;
	
	_backspace_button = _create_button(self, "Del", 11+1, 1, 2, 1);
	...
	y = 1;
	# standard buttons
	for line in BUTTON_LAYOUT_FRENCH:
		for k in line:
